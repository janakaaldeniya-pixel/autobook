// components/marketplace/ImageUploader.tsx
'use client';
...
export function ImageUploader({
  ...
  name: string;      // hidden form field name the server action reads
  maxImages?: number;
}) {
  ...
  function removeImage(url: string) {
    // Removes it from this listing only — doesn't delete the file from
    // storage. An orphaned file costs nothing to worry about for v1; add a
    // cleanup job later if storage usage ever becomes a concern.
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {images.map((url, i) => (
          <div
            key={url}
            className="relative aspect-square overflow-hidden rounded-sm border border-[#3A4249] bg-[#1B2023]"
          >
